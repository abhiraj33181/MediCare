...
function Testinomials() {
  return (
    <section className='py-16 px-4 dm:px-6 lg:px-8 bg-background'>
            ...
            <div className='text-center mb-12'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
                    {testimonials.map((testinomial, index) => (
                        <Card key={index} className={`${testinomial.bgColor} border border-border hover:shadow-lg transition-shadow duration-200`}>
                            <CardContent className='px-6'>
                                <div className='flex text-yellow-400'>
                                    {[...Array(testinomial.rating)].map((_, i) => (
                                    <svg key={i} className="w-6 h-6 fill-current" viewBox="0 0 24 24">
                                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                                    </svg>
                                ))}
